...
class SlaveCmd(TalusCmdBase):
    ...
    def do_delete(self, args):
        """Delete a slave within Talus

        slave delete <SLAVE_ID_OR_NAME>
        """
        if args.strip() == "":
        ...
        slave = self._resolve_one_model(slave_id_or_name, Slave, search, default_id_search=["hostname", "id", "ip"],
                                        sort="hostname")

        self._talus_client.slave_delete(slave.id)
        print("deleted")
    ...
    def do_info(self, args):
        ...
        if slave is None:
            raise talus_client.errors.TalusApiError(
                "Could not locate slave by id/hostname/ip {!r}".format(slave_id_or_name))
